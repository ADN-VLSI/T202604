...
module tb_regif;
    ...
    task read_and_check(input logic [2:0] addr, input logic [31:0] expected_data, input logic expected_error);
        @(posedge clk_i);
        addr_i = addr;
        we_i   = 1'b0;
        ...
        if (rdata_o === expected_data && error_o === expected_error) begin
            $display("[SUCCESS] Addr: 0x%0h | Read Data: 0x%0h (Expected: 0x%0h) | Error: %b", addr, rdata_o, expected_data, error_o);
        end else begin
            $display("[ERROR MISMATCH] Addr: 0x%0h | Read Data: 0x%0h (Expected: 0x%0h) | Error: %b (Expected: %b)", 
